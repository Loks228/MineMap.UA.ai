...
async function loadRegions() {
    try {
        ...
        // Заполняем выпадающий список регионов для фильтра
        ...
        // Заполняем выпадающий список для экспорта
        ...
        if (exportRegion) {
            regions.forEach(region => {
                ...
                option.value = region.id;
                option.textContent = region.name;
                exportRegion.appendChild(option);
            });
        }
        
        // Заполняем выпадающий список регионов для добавления/редактирования объекта
        const regionSelect = document.getElementById('region');
        if (regionSelect) {
            // Очищаем текущие опции
            regionSelect.innerHTML = '';
            
            // Добавляем опции регионов
            regions.forEach(region => {
                const option = document.createElement('option');
                option.value = region.id;
                option.textContent = region.name;
...
async function saveObject() {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.detail || 'Произошла ошибка при сохранении объекта');
        }
...
async function deleteObjectConfirm() {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.detail || 'Произошла ошибка при удалении объекта');
        }
